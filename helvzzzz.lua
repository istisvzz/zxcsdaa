repeat task.wait() until game.CreatorId == 10611639
repeat task.wait() until game:IsLoaded() == true
repeat task.wait() until game.Workspace:FindFirstChild(game.Players.LocalPlayer.Name)

local __checker = loadstring(game:HttpGet("https://raw.githubusercontent.com/buang5516/urban-octo-fishstick/main/yes.lua"))()
repeat task.wait() until __checker == true
...
makefolder('Buang')
makefolder('Buang/AnimeAdventures')
...
local PortalsList = {'Alien Portal', 'Noble Portal', 'Eclipse Portal','Port Agency Portal', 'Puppet Portal', "Demon Leader's Portal"}
...
local portal_names = {
	portal_item__bsd		= 'Port Agency Portal',
	portal_item__eclipse	= 'The Eclipse',
	portal_item__fate		= 'Noble Portal',
}
...
local portalWorlds = {        
    ...
    ['7ds']         ='Fabled Kingdom',
    ['mha']         = 'Hero City',
    ['dressrosa']   = 'Puppet Island',
    ['sao']         = 'Virtual Dungeon',
    ['berserk']     = 'Windhym',
    ['overlord']    = 'Undead Tomb',
    ['fate']        = 'Mountain Temple',
    ['amegakure']   = 'Rain Village'

	--[[morioh_halloween = 'Bizarre Town (Haunted)',
	...
	fate_portal = 'Holy Grail War', -- Holy Grail War
...
local Settings = {
        ...
        ['Colored Takedowns'] = false;

		['AutoPlacePositions'] = {};
		['AutoPlacesCap'] = {['1'] = 4, ['2'] = 4, ['3'] = 4, ['4'] = 4, ['5'] = 4, ['6'] = 4};
		['AutoUpgradesCap'] = {['1'] = 10, ['2'] = 10, ['3'] = 10, ['4'] = 10, ['5'] = 10, ['6'] = 10};

        ['Auto Buff Erwin 100%'] = false;
        ['Auto Buff Wenda 100%'] = false;
        ['Auto Buff Leafy 100%'] = false;
        ...
        ['Auto Next Portal'] = false;
        ['Place In Red Zones'] = false;
        ['Hide Map'] = false;
        ['Auto Open Capsules'] = false;
        ['Auto Delete Skins'] = false;
        ['Auto Delete Skins'] = false;

        ['Fake Outfit'] = false;
        ['Auto Delete Portals'] = false;
        ['Hard Difficulty'] = false;
        ['Auto Join Story/INF'] = false;
        ['Only Friends'] = false;
    ...
    local newSetTable = {'JoinMode', 'Join Players'}

local fileName = 'Buang/Settings_' .. player.Name .. '.json'

function saveSettings()
    if not isfolder('Buang') then
        makefolder('Buang')
    end
    writefile('Buang/Settings_' .. game.Players.LocalPlayer.Name .. '.json', HttpService:JSONEncode(Settings))
...
function ReadSetting()
    local s, e = pcall(function()
        ...
        return HttpService:JSONDecode(readfile('Buang/Settings_' .. game.Players.LocalPlayer.Name .. '.json'))
    end)
    if s then
        return e
    else
...
local function GetSave (valueName, ty)


    local value
    if not Settings[valueName] then
        Settings[valueName] = false
		if ty ~= nil and ty == 'table' then 
        	value = {}
		else
			value = false
		end
        if table.find(newSetTable, valueName) then
            value = {}
        else
            value = false
        end
        ...
        if table.find(newSetTable, valueName) then
            if typeof(Settings[valueName]) == 'table' then
...
DiscordLabel.Text = "Discord: XQydrhg8FN"
...
function getMacroUnits(macroName, textBlocks)
	...
	local macroExist = isfile('Buang/AnimeAdventures/' .. nameMacro)
	local macroExistjson = isfile('Buang/AnimeAdventures/' .. nameMacro .. '.json')
	...
	if macroExist or macroExistjson then
		...
		local sucess, response = pcall(function()
			local MacroAbout = HttpService:JSONDecode( readfile( 'Buang/AnimeAdventures/' .. nameMacro) )
...
Farm_HalloweenSubPage = MakeNewSubPage('Farm', 'Right', 0.157, 0.1, 0.05, 0.08)
MakeTitle(Farm_HalloweenSubPage, 'Auto Join Halloween', 0.27)
AutoHalloween = MakeCheckbox(Farm_HalloweenSubPage, 'Auto Halloween', 0.23)
HalloweenSolo = MakeCheckbox(Farm_HalloweenSubPage, 'Solo Only Halloween', 0.23)
...
local Misc_AutoJoinPlayer = MakeNewSubPage('Misc', 'Left', 0.405, 0.03, 0.02, 0.01)
MakeTitle(Misc_AutoJoinPlayer, 'Auto Join Player', 0.105)
local selectTypeJoin = MakeDDL(Misc_AutoJoinPlayer, 'Join Mode', 0.26)
local selectPlayerDDL = MakeDDL(Misc_AutoJoinPlayer, 'Select Player', 0.26)
local autoJoinPlayer = MakeCheckbox(Misc_AutoJoinPlayer, 'Enable Auto Join PLR', 0.10)

local copyDiscordLink = MakeLargeButton(Misc_AutoJoinPlayer, 'Copy Discord Link', 0.103)
...
local function DeletePortalsFunc ()
	...
	while GetSave(AutoDeletePortals.Name) do
		...
		for _, itemData in pairs(RefreshedUniqueItems) do
			...
			if portalWorld then 
                local __map = string.match(portalWorld, '[%l]+')
                if table.find(IgnoreWorlds, __map) then
                    continue 
                end
            end
...
HideUHLabel.Text = 'Hide [BUANG] Hub'
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['color'] = 11513855,
				['footer'] = {
					['text'] = string.format("// Skidded by Buang Hub (%s)", Time), 
				},
				['fields'] = {
					{
						['name'] = ':scream_cat::scream: Test Webhook :scream::scream_cat:',
						['value'] = string.format(stt, 'Alucard', '999')
					}
				},
...
for tabName, mapsList in pairs(macroMapList) do
	...
	for _, mapName in ipairs(mapsList) do
		...
		newDDLMacroMap.MouseButton1Click:Connect(function()
			...
			local MacrosList = listfiles('Buang/AnimeAdventures')

			local items = {}

			for _, macro in ipairs(MacrosList) do
				local macroName = string.match(string.sub(macro, 23), "[^.]+")
...
local function RecordMacroFunc ()
	...
	local macroExist = isfile('Buang/AnimeAdventures/' .. MacroFile )
	local macroExistJson = isfile('Buang/AnimeAdventures/' .. MacroFile .. '.json')

	if macroExistJson then MacroFile = 'Buang/AnimeAdventures/' .. MacroFile .. '.json'
	elseif macroExist then MacroFile = 'Buang/AnimeAdventures/' .. MacroFile
...
local function PlayMacroFunc ()
	...
	elseif LevelData._is_map_or_portal_level then
		local __mappy = nil
		for _,v in pairs(getLevelData()) do
			if table.find(portal_names, _) then
				__mappy = v
		...
		MacroFile = GetSave('Selected Macro Map')['Portal'][__mappy]
		end
	else
	...
	if MacroFile['Auto Sort Config'] then 
		MacroFile['Auto Sort Config'] = nil
	end

	for actionOrder, AboutAction in pairs(MacroFile) do
		if type(AboutAction) ~= 'table' then continue end

		if actionOrder == 'Auto Sort Config' then continue end
...
createMacroBox.FocusLost:Connect(function(enterPressed)
	if not enterPressed and (createMacroBox.Text ~= '' or createMacroBox.Text ~= ' ') then return end

	writefile('Buang/AnimeAdventures/' .. createMacroBox.Text .. '.json', HttpService:JSONEncode({}))
end)
...
deleteMacroButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Buang/AnimeAdventures/' .. item)
	local macroExistjson = isfile('Buang/AnimeAdventures/' .. item .. '.json')
	if macroExistjson then item = item .. '.json' end

	if not macroExist and not macroExistjson then return end

	delfile('Buang/AnimeAdventures/' .. item)
...
equipMacroUnitsButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Buang/AnimeAdventures/' .. macroSelected)
	local macroExistjson = isfile('Buang/AnimeAdventures/' .. macroSelected .. '.json')
	...
	local MacroAbout = HttpService:JSONDecode( readfile( 'Buang/AnimeAdventures/' .. macroSelected) )
...
selectTypeJoin.MouseButton1Click:Connect(function()

	local __items = {'Raids', 'Story', 'Portals', 'Challenges', 'Event', 'Everything'};

	GetDDL(selectTypeJoin, __items, false, 'JoinMode')

end)

DDLlabel(selectTypeJoin, GetSave('JoinMode', 'table'))


selectPlayerDDL.MouseButton1Click:Connect(function()

	local items = GetSave('Join Players', 'table')
	...
	GetDDL(selectPlayerDDL, items, true, 'Join Players')

end)

DDLlabel(selectPlayerDDL, GetSave('Join Players', 'table'))
...
autoUseAbilityUnitsTable = {'hidan_unit_evolved', 'stain_evolved','dazai_evolved', 'alucard_evolved', 'jotaro', 'eren_final', 'ainz_evolved', 'kisuke_evolved',
...
checkBoxFunc(AutoHalloween)
checkBoxFunc(autoJoinPlayer)
checkBoxFunc(HalloweenSolo)
...
copyDiscordLink.MouseButton1Click:Connect(function()
	local link = loadstring(game:HttpGet("https://raw.githubusercontent.com/buang5516/urban-octo-fishstick/main/discord.lua"))() or "https://discord.gg/XQydrhg8FN"
...
local function getMapName (result)
	...
	if LevelData.id == 'halloween2_event' then
		mapName = string.format('Map: %s\n**%s**', LevelData._location_name, result)
	end
...
local RareDrop = {"Detective Shard", 'Path Portal'}
...
local oldPlayerStats = {
	...
	Candies = player._stats._resourceCandies.Value,
}
...
local function newSecretItemFunc (itemName, Amount)
	...
	contentNewSecretItem = contentNewSecretItem .. '```'
end
...
local function webhook ()
	...
	local newResources = {
		...
		{name = 'Halloween Candies', amount = player._stats._resourceCandies.Value - oldPlayerStats.Candies}
	...
	for itemId, itemData in pairs(newItemsData) do
		if not oldItemsData[itemId] then 
			...
			newItemsTable[itemData.Name] = itemData.Amount
            newItemsOld[itemData.Name] = itemData.Amount
             continue
	...
	local TotalCandies = makeComma(player._stats._resourceCandies.Value)
	local BattlePass = ''
	local reachedTier = 0
	...
	for itemName, itemAmount in pairs(newItemsTable) do
		...
		newItems = newItems .. string.format('+%s (%s) {Total: %s}', itemName, itemAmount, newItemsOld[itemName]) .. comma
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['footer'] = {
					['text'] = string.format("// Skidded by Buang Hub (%s)", os.date("%X")), 
				},
				['fields'] = {
					{
						['name'] = "Player Stats",
						['value'] = string.format( "<:Gems:1148368507029950515> %s\n<:Gold:1148368511463338074> %s\n<:candies:1179742900368707735> %s\n:tickets: Tier: %s", TotalGems, TotalGold, TotalCandies, BattlePass),
						['inline'] = true
					},
...
task.spawn(function() 
	pcall(function()
		...
		if queue_on_teleport then
			local UltraHubSCRIPT = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/buang5516/urban-octo-fishstick/main/buanghub"))()'
...
local function leaving__webhook ()
	local ping = GetSave(PingUserCHB.Name)
	local userID = string.format("<@%s>", GetSave("Discord UserID") )
	local webhookUrl = GetSave("Discord Url")
    ...
    local function __getMapName (result)
        local mapName = result
        ...
        elseif LevelData._gamemode ~= 'infinite' then
            mapName = LevelData.name .. ' - PLAYER DISCONNECTED'
        end
	...
	local result = __getMapName(ResultUI.Holder.Title.Text)
    if GetSave(PingDefeatCHB.Name) and LevelData._gamemode ~= 'infinite' then ping = true end
	local newGems = ""
	local TotalGems = makeComma(player._stats.gem_amount.Value)
	local TotalGold = makeComma(player._stats.gold_amount.Value)
	local TotalCandies = makeComma(player._stats._resourceCandies.Value)
	...
	local data = {
		...
		["content"] = "YOU GOT DISCONNECTED " .. userID,
		["embeds"] = {
...
game.Players.PlayerRemoving:Connect(function(plr)
    if plr == game.Players.LocalPlayer and __ended == false and not IsLobby and (GameFinished) and not GameFinished.Value and not ResultUI.Enabled  then
        if GetSave(ResultWebhook.Name) then
            pcall(function() leaving__webhook() end)
        end
    end
end)
...
if IsLobby then
	...
	while true  do
		...
		local autoJoinPlayerNeed = false

		if GetSave(autoJoinPlayer.Name) then
			autoJoinPlayerNeed = true;

			local joinMode = GetSave('JoinMode') -- Raids Story Portals Challenges Event Everything
			local playerToJoin = GetSave('Join Players')

			local function __portals()
				for i,v in pairs(workspace._PORTALS.Lobbies:GetDescendants()) do
					
					if v.Name == 'Owner' and table.find(playerToJoin, tostring(v.value)) then
						Event['request_join_lobby']:InvokeServer(v.Parent.Name)
					end
				end
			end

			local function __raids()
			...
			local function __event()
				for i,v in pairs(workspace._EVENT_CHALLENGES.Lobbies:GetDescendants()) do
					if v.Name == 'Owner' and table.find(playerToJoin, tostring(v.value)) then
					Event['request_join_lobby']:InvokeServer(v.Parent.Name)
					end 
				end
			...
			if autoJoinPlayerNeed then 

				if joinMode == 'Raids' then
					__raids()
				elseif joinMode == 'Story' then
					__story()
				elseif joinMode == 'Portals' then 
					__portals()
				elseif joinMode == 'Challenges' then
					__challenges()
				elseif joinMode == 'Event' then
					__event()
				elseif joinMode == 'Everything' then
					__raids()
					__story()
					__portals()
					__challenges()
		...
		if GetSave(AutoChallenge.Name) and not autoJoinPlayerNeed then
			challengeNeed = true
		...
		local halloweenNeed = false

		if GetSave(AutoHalloween.Name) and not challengeNeed and not autoJoinPlayerNeed then
			halloweenNeed = true;

			if halloweenNeed then

				for i,v in pairs(workspace._EVENT_CHALLENGES.Lobbies:GetChildren()) do
					if (GetSave(HalloweenSolo.Name)) and (#v.Players:GetChildren() >0 or v.Active.Value) then continue end
					Event['request_join_lobby']:InvokeServer(v.Name)
					player.Character:SetPrimaryPartCFrame(CFrame.new(Vector3.new(255.16964721679688, 196.8419189453125, -820.8120727539062)))

					if GetSave(HalloweenSolo.Name) then
						while #v.Players:GetChildren() == 1 and v.Timer.Value > 0 do
							task.wait(0.05)
						end

						if #v.Players:GetChildren() ~= 1 then
							Event['request_leave_lobby']:InvokeServer(v.Name)
							player.Character:SetPrimaryPartCFrame(CFrame.new(Vector3.new(255.16964721679688, 196.8419189453125, -820.8120727539062)))
						end
						break
					else
						while v.Timer.Value > 0 do
							task.wait(0.05)
						end
		...
		if GetSave(StoryInf_AutoJoin.Name) and not challengeNeed and not halloweenNeed and not autoJoinPlayerNeed then
		...
		if GetSave(InfCastleAutoJoin.Name) and not storyInfNeed and not halloweenNeed and not autoJoinPlayerNeed then
			CastleJoined = true
		...
		if GetSave(AutoUsePortal.Name) and not challengeNeed and not CastleJoined and not storyInfNeed and not halloweenNeed and not autoJoinPlayerNeed then
